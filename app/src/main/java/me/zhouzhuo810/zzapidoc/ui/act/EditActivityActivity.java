...
//        cbShowTitle.setChecked(true);
            ...
            switch (requestCode) {
                ...
                case REQUEST_SELECT_PICTURE:
                    ...
                    if (selectedUri != null) {
                        ...
                        splashPath = ContentUtils.getRealPathFromUri(EditActivityActivity.this, selectedUri);
                    } else {
                ...
                case REQUEST_SELECT_PICTURE_GUIDE_ONE:
                    ...
                    if (selectedUriOne != null) {
                        String path = ContentUtils.getRealPathFromUri(EditActivityActivity.this, selectedUriOne);
                ...
                case REQUEST_SELECT_PICTURE_GUIDE_TWO:
                    ...
                    if (selectedUriTwo != null) {
                        String path = ContentUtils.getRealPathFromUri(EditActivityActivity.this, selectedUriTwo);
                        zibPic.addImage(path);
                    } else {
                        ToastUtils.showCustomBgToast("选择图片出错了，请选择其他图片");
                    }
                ...
                case REQUEST_SELECT_PICTURE_GUIDE_THREE:
                    ...
                    if (selectedUriThree != null) {
                        String path = ContentUtils.getRealPathFromUri(EditActivityActivity.this, selectedUriThree);
                        zibPic.addImage(path);
                    } else {
                        ToastUtils.showCustomBgToast("选择图片出错了，请选择其他图片");
                    }
                ...
                case REQUEST_SELECT_PICTURE_GUIDE_FOUR:
                    ...
                    if (selectedUriFour != null) {
                        String path = ContentUtils.getRealPathFromUri(EditActivityActivity.this, selectedUriFour);
                        zibPic.addImage(path);
                    } else {
                        ToastUtils.showCustomBgToast("选择图片出错了，请选择其他图片");
                    }
                ...
                case REQUEST_SELECT_PICTURE_GUIDE_FIVE:
                    ...
                    if (selectedUriFive != null) {
                        String path = ContentUtils.getRealPathFromUri(EditActivityActivity.this, selectedUriFive);
                        zibPic.addImage(path);
                    } else {
                        ToastUtils.showCustomBgToast("选择图片出错了，请选择其他图片");
                    }
    ...
    private void addAct() {
        ...
        PostRequest<AddActivityResult> post = OkGo.<AddActivityResult>post(SharedUtil.getString(ZApplication.getInstance(), "server_config")
                + "/ZzApiDoc/v1/activity/updateActivity")
                .params("actId", actId)
                .params("name", name)
                .params("title", title)
    ...
    private void chooseType(int position) {
        List<String> items = new ArrayList<>();
        items.add("Empty Activity");
        items.add("Splash Activity");
        items.add("Guide Activity");
        ...
        actType = position;
        tvActType.setText(items.get(position));
    }
    ...
    private void getData() {
        ...
        Api.getApi0()
                .getActivityDetail(getUserId(), actId)
                .compose(RxHelper.<GetActivityDetailResult>io_main())
                .subscribe(new Subscriber<GetActivityDetailResult>() {
                    @Override
                    ...
                    @Override
                    public void onNext(GetActivityDetailResult getActivityDetailResult) {
                        hidePd();
                        if (getActivityDetailResult.getCode()==1) {
                            fillData(getActivityDetailResult.getData());
                        } else {
                            ToastUtils.showCustomBgToast(getActivityDetailResult.getMsg());
                        }
                    }
                });
    }

    private void fillData(GetActivityDetailResult.DataEntity data) {
        if (data != null) {
            appId = data.getAppId();
            etActTitle.setText(data.getTitle());
            tvActName.setText(data.getName());
            etSplashDuration.setText(data.getSplashSecond()+"");
            targetActId = data.getTargetActId();
            tvTargetAct.setText(data.getTargetActName());
            cbIsFirst.setChecked(data.isFirst());
            cbIsFullScreen.setChecked(data.isFullScreen());
            cbIsLandscape.setChecked(data.isLandscape());
            chooseType(data.getType());
        }
    }
