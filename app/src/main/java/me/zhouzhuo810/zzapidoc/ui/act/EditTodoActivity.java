...
public class EditTodoActivity extends BaseActivity {
    ...
    private GetAllMyTodoListWebResult.DataEntity dataEntity;
    ...
    public void initData() {
        dataEntity = getIntent().getParcelableExtra("todo");
        if (dataEntity != null) {
            todoId = dataEntity.getId();
            etTodoTitle.setText(dataEntity.getContent());
            etTodoContent.setText(dataEntity.getNote());
            tvProgress.setText(dataEntity.getProgress() + "%");
            seekProgress.setProgress(dataEntity.getProgress());
            handlerPersonId = dataEntity.getHandlePersonId();
            tvHandlePerson.setText(dataEntity.getHandlePersonName());
        }
    }
    ...
    private void submit() {
        ...
        Api.getApi0()
                .updateTodoList(todoId, getUserId(), handlerPersonId, title, content, seekProgress.getProgress(), seekProgress.getProgress() == 100 ? 1 : 0)
                .compose(RxHelper.<UpdateTodoListResult>io_main())
                .subscribe(new Subscriber<UpdateTodoListResult>() {
                    @Override
                    public void onCompleted() {
                    ...
                    @Override
                    public void onNext(UpdateTodoListResult addTodoListResult) {
                        hidePd();
